...
namespace Startran.Mod
{
    internal static class SdvDialogueFixer
    {
        ...
        {
            // Console.WriteLine(@"Try finding tokens...");
            var originalTokens = originalText.Split(new[] { SeparatorToken }, StringSplitOptions.None);
            var translatedTokens = translatedText.Split(new[] { SeparatorToken }, StringSplitOptions.None);

#if DEBUG
            // Try finding defaultLang's ##
            ...
            for (var i = 0; i < originalTokens.Length; i++)
            {
                ...
                Console.WriteLine(@$"Token index: {i}, Original order: {string.Join(", ", originalSymbolOrder)}, Translated order: {string.Join(", ", translatedSymbolOrder)}");
                return true;
            }
